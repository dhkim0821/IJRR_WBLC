...
MinJerk_OneDimension::MinJerk_OneDimension(const dynacore::Vect3 & init, const dynacore::Vect3 & end, 
...
void MinJerk_OneDimension::Initialization(){
	...
	C_mat = dynacore::Matrix::Zero(6,6);
	C_mat_inv = dynacore::Matrix::Zero(6,6);
	a_coeffs = dynacore::Vector::Zero(6);
	bound_cond = dynacore::Vector::Zero(6);

	init_cond = dynacore::Vector::Zero(3);
	end_cond = 	dynacore::Vector::Zero(3);
	to = 0.0;
	tf = 1.0;
}

void MinJerk_OneDimension::setParams(const dynacore::Vect3 & init, const dynacore::Vect3 & end, 
 			  const double & time_start, const double & time_end){
	// Set the Parameters
	init_cond = init;
	end_cond = end;
...
void MinJerk_OneDimension::printParameters(){
	...
	dynacore::pretty_print(init_cond, std::cout, "init_cond");
	dynacore::pretty_print(end_cond, std::cout,  "end_cond");	
	dynacore::pretty_print(C_mat, std::cout, "Coeff_Matrix");
	dynacore::pretty_print(C_mat_inv, std::cout, "C_mat_inv");

	dynacore::pretty_print(a_coeffs, std::cout, "Coeff_Vector");	
	dynacore::pretty_print(bound_cond, std::cout, "Boundary Conditions");
}
