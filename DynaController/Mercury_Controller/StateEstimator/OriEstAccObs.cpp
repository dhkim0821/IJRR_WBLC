...
void OriEstAccObs::EstimatorInitialization(const dynacore::Quaternion & ini_quat,
...
void OriEstAccObs::setSensorData(const std::vector<double> & acc,
  ...
  x_pred_.head(6) += delta.head(6);
  x_pred_.tail(3) += delta.tail(3);

  dynacore::Vect3 delta_ori = delta.segment(6,3);
  ...
  dynacore::Matrix eye(DIM_STATE_EST_ACC_OBS, DIM_STATE_EST_ACC_OBS);
  eye.setIdentity();
