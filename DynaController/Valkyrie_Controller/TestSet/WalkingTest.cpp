...
#include <Valkyrie_Controller/CtrlSet/DoubleContactTransCtrl.hpp>
#include <Valkyrie_Controller/CtrlSet/BodyCtrl.hpp>
#include <Valkyrie_Controller/CtrlSet/SingleContactTransCtrl.hpp>
#include <Valkyrie_Controller/CtrlSet/BodyFootPlanningCtrl.hpp>
    ...
    num_step_(0)
{
    ...
    right_swing_start_trans_ctrl_ = 
        new SingleContactTransCtrl(robot, valkyrie_link::rightFoot, false);
    right_swing_end_trans_ctrl_ = 
        new SingleContactTransCtrl(robot, valkyrie_link::rightFoot, true);
    // Left
    left_swing_start_trans_ctrl_ = 
        new SingleContactTransCtrl(robot, valkyrie_link::leftFoot, false);
    left_swing_end_trans_ctrl_ = 
        new SingleContactTransCtrl(robot, valkyrie_link::leftFoot, true);
    // Swing Controller Selection
    right_swing_ctrl_ = 
        new BodyFootPlanningCtrl(robot_sys_, valkyrie_link::rightFoot, reversal_planner_);
    left_swing_ctrl_ = 
        new BodyFootPlanningCtrl(robot_sys_, valkyrie_link::leftFoot, reversal_planner_);
...
void WalkingTest::TestInitialization(){
    ...
    reversal_planner_->PlannerInitialization(
            ValkyrieConfigPath"PLANNER_velocity_reversal");
    // Yaml file name
...
void WalkingTest::_SettingParameter(){
    // Setting Parameters
    ParamHandler handler(ValkyrieConfigPath"TEST_walking.yaml");
