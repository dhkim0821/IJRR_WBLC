#include "LinkGlobalSelectPosTask.hpp"
// (X, Y, Z)
...
LinkGlobalSelectPosTask::LinkGlobalSelectPosTask(
        const RobotSystem* robot, int link_idx, int selected_dim):
    KinTask(1),
    robot_sys_(robot), link_idx_(link_idx), selected_dim_(selected_dim)
{
...
bool LinkGlobalSelectPosTask::_UpdateCommand(void* pos_des,
    ...
    pos_err_[0] = (*pos_cmd)[selected_dim_] - 
        (link_pos[selected_dim_] + sp_->global_pos_local_[selected_dim_]);
...
bool LinkGlobalSelectPosTask::_UpdateTaskJacobian(){
...
bool LinkGlobalSelectPosTask::_UpdateTaskJDotQdot(){
    dynacore::Vector tmp;
