    ...
    Kd_(nao::num_act_joint)
{
    ...
    body_pos_task_ = new LinkPosTask(robot_sys_, nao_link::torso);
    body_ori_task_ = new LinkOriTask(robot_sys_, nao_link::torso);
    ...
    wblc_data_->W_qddot_ = dynacore::Vector::Constant(nao::num_qdot, 10.0);
    wblc_data_->W_rf_ = dynacore::Vector::Constant(dim_contact_, 1.0);
    wblc_data_->W_xddot_ = dynacore::Vector::Constant(dim_contact_, 100.0);
    ...
    int rf_idx_offset(0);
    if(swing_foot == nao_link::l_ankle) {
    ...
    else if(swing_foot == nao_link::r_ankle) {
...
void BodyFootPlanningCtrl::_task_setup(){
    ...
    dynacore::Vector zero(nao::num_act_joint); zero.setZero();
...
void BodyFootPlanningCtrl::_Replanning(dynacore::Vect3 & target_loc){
    ...
    if(swing_foot_ == nao_link::l_ankle)
...
void BodyFootPlanningCtrl::FirstVisit(){
    ...
    robot_sys_->getOri(swing_foot_, ini_foot_quat_);
    ...
    //dynacore::pretty_print(ini_foot_pos_, std::cout, "ini foot");
...
bool BodyFootPlanningCtrl::EndOfPhase(){
    ...
    if(b_contact_switch_check_){
        ...
        if(swing_foot_ == nao_link::l_ankle && sp_->b_lfoot_contact_){
            contact_happen = true;
        }
        if(swing_foot_ == nao_link::r_ankle && sp_->b_rfoot_contact_){
            contact_happen = true;
        }
