...
Atlas_StateEstimator::Atlas_StateEstimator(RobotSystem* robot):
    curr_config_(atlas::num_q),
    curr_qdot_(atlas::num_qdot)
{
...
Atlas_StateEstimator::~Atlas_StateEstimator(){
    delete ori_est_;
}

void Atlas_StateEstimator::Initialization(Atlas_SensorData* data){
    curr_config_.setZero();
    curr_qdot_.setZero();
    curr_config_[atlas::num_qdot] = 1.;

    // Joint Set
    for (int i(0); i<atlas::num_act_joint; ++i){
        curr_config_[atlas::num_virtual + i] = data->jpos[i];
        curr_qdot_[atlas::num_virtual + i] = data->jvel[i];
    }
    ...
    curr_config_[atlas::num_qdot] = body_ori.w();
...
void Atlas_StateEstimator::Update(Atlas_SensorData* data){
    curr_config_.setZero();
    curr_qdot_.setZero();
    curr_config_[atlas::num_qdot] = 1.;
