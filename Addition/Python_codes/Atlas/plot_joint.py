...
def create_figures(subfigure_width=480, subfigure_height=600, starting_figure_no=1, starting_col_index = 0, starting_row_index=0, plot_configuration=PLOT_HORIZONTALLY):
    ...
    for i in range(1,num_leg_joint+1,1):
        ...
        plt.plot(data_x, data_jpos_des[st_idx:end_idx,i-1 + leg_jidx_start], "r-", \
                data_x, data_jjpos[st_idx:end_idx, i-1 + leg_jidx_start], "c-")
    ...
    for i in range(1,num_leg_joint + 1,1):
        ax1 = plt.subplot(num_leg_joint, 1, i)
        plt.plot(data_x, data_jpos_des[st_idx:end_idx,i-1 + num_leg_joint + leg_jidx_start], "r-" , \
                data_x, data_jjpos[st_idx:end_idx, i-1 + num_leg_joint + leg_jidx_start], "c-")
        # plt.legend(('command', 'pos'), loc='upper left')
        # phase marker #
    ...
    for i in range(1,num_leg_joint + 1,1):
        ...
        plt.plot(data_x, data_qdot[st_idx:end_idx,i-1 + 6 + leg_jidx_start], "b-", \
                 data_x, data_jvel_des[st_idx:end_idx, i-1 + leg_jidx_start], "r-")
    ...
    for i in range(1,num_leg_joint + 1,1):
        ax1 = plt.subplot(num_leg_joint, 1, i)
        plt.plot(data_x, data_jjvel[st_idx:end_idx, i-1 + num_leg_joint], \
                color="green", linewidth=1.3)
        plt.plot(data_x, data_qdot[st_idx:end_idx,i-1 + 6 + num_leg_joint + leg_jidx_start], "b-", \
                 data_x, data_jvel_des[st_idx:end_idx, i-1 + num_leg_joint + leg_jidx_start], "r-");
