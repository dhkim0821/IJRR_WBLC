...
simulation_setting * sim_setting = new simulation_setting();
Dyn_environment* gDyn_env = new Dyn_environment();

extern std::vector<Vec3> g_trajectory;
extern std::vector<Vec3> g_leftfoot;
extern std::vector<Vec3> g_rightfoot;

void User_Simulation_Go_One_Step()
{
    g_trajectory.push_back((gDyn_env->m_Hume->m_Link[HumeID::SIM_HIP]).GetFrame().GetPosition());
    g_leftfoot.push_back((gDyn_env->m_Hume->m_Link[HumeID::SIM_LFOOT]).GetFrame().GetPosition());
    g_rightfoot.push_back((gDyn_env->m_Hume->m_Link[HumeID::SIM_RFOOT]).GetFrame().GetPosition());
...
void User_CBFunc_Forward(){
    // HUME_System::GetHumeSystem()->state_provider_->attraction_loc_[0] += 0.03;
}
void User_CBFunc_Backward(){
    // HUME_System::GetHumeSystem()->state_provider_->attraction_loc_[0] -= 0.03;
}
void User_CBFunc_Left(){
    // HUME_System::GetHumeSystem()->state_provider_->attraction_loc_[1] += 0.03;
}
...
void User_SimulationSetting()
{
	...
	gViewer.SetKeyFunc(User_CBFunc_Pause_DYN, 'o');
        ...
        gViewer.SetKeyFunc(User_CBFunc_Left, 'a');
        gViewer.SetKeyFunc(User_CBFunc_Right, 'd');
        gViewer.SetKeyFunc(User_CBFunc_Backward, 's');
...
int main(int argc, char **argv) 
{
    ...
    gViewer.Init(&argc, argv, "Dynamic Simulation of Hume");
